...
def plot_boundary_points(self, variable=None, ax=None, figsize=(10, 8), **kwargs):
    ...
    **kwargs : dict
    ...
    boundary = None
    if hasattr(self.data, 'ocean') and self.data.ocean is not None and hasattr(self.data.ocean, 'boundary') and self.data.ocean.boundary is not None:
    ...
    # Fall back to legacy location for backward compatibility
        ...
        raise ValueError("No boundary data available in the config object (checked both data.ocean.boundary and data.boundary)")
    ...
    if variable is not None and variable in dataset:
        ...
        if 'node' in dataset.dims and len(dataset[variable].shape) >= 1:
            ...
            logger.warning(f"Variable {variable} not suitable for coloring boundary points")
...
def plot_boundary_timeseries(self, variable=None, boundary_idx=0, node_idx=0, 
                           ax=None, figsize=(12, 6), **kwargs):
    ...
    **kwargs : dict
    ...
    if boundary_idx >= grid.pylibs_hgrid.nob:
        raise ValueError(f"Boundary index {boundary_idx} out of range (0-{grid.pylibs_hgrid.nob-1})")
    
    boundary_nodes = grid.pylibs_hgrid.iobn[boundary_idx]
    if node_idx >= len(boundary_nodes):
        raise ValueError(f"Node index {node_idx} out of range (0-{len(boundary_nodes)-1})")
    ...
    if 'node' in dataset.coords:
        ...
        if global_node_idx in dataset_nodes:
            ...
            logger.warning("Node indices in dataset do not match grid node indices. Using local index.")
    ...
    ax.set_title(f'{variable} at Boundary {boundary_idx}, Node {node_idx} ({x:.2f}, {y:.2f})')
...
def plot_boundary_profile(self, variable=None, boundary_idx=0, node_idx=0, time_idx=0,
                        ax=None, figsize=(8, 10), **kwargs):
    ...
    **kwargs : dict
    ...
    if variable not in dataset or depth_dim not in dataset[variable].dims:
        raise ValueError(f"Variable '{variable}' not found in dataset or does not have a depth dimension")
    ...
    if 'time' in dataset[variable].dims:
        values = dataset[variable].isel(time=time_idx, **{node_dim: dataset_node_idx}).values
    else:
        values = dataset[variable].isel({node_dim: dataset_node_idx}).values
