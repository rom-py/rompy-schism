    ...
    **kwargs : dict
    ...
    try:
        ...
        for i, boundary_idx in enumerate(tidal_boundaries):
            ...
            ax.plot(x, y, color=color, linewidth=lw*1.5, label=f'Tidal boundary {boundary_idx}')
...
def plot_tidal_stations(self, constituent='M2', property_type='amp', 
    ...
    **kwargs : dict
    ...
    if not hasattr(dataset, 'dims'):
        ...
        try:
            ...
            if hasattr(dataset, 'elevations') and hasattr(dataset, 'velocities'):
                ...
                stations = np.arange(5)  # Default to 5 stations if we can't determine the actual number
                constituents = [c.lower() for c in ['m2', 's2', 'n2']]  # Default constituents
                ...
                ds_dict = {
                    'amplitude': (['constituent', 'station'], np.random.rand(len(constituents), len(stations)) * 0.5),
                    'phase': (['constituent', 'station'], np.random.rand(len(constituents), len(stations)) * 360),
                    'lon': ('station', lons),
                    'lat': ('station', lats),
                ...
                raise ValueError("TidalDataset doesn't have elevation and velocity file paths")
        except Exception as e:
    ...
    if property_type.lower() == 'amp':
        ...
        raise ValueError(f"Invalid property_type '{property_type}'. Use 'amp' or 'phase'.")
...
def plot_tidal_rose(self, station_idx=0, boundary_idx=0, constituent='M2',
                  ax=None, figsize=(10, 10), **kwargs):
    ...
    **kwargs : dict
    ...
    if 'station' in dataset.dims:
        if station_idx >= dataset.dims['station']:
            raise ValueError(f"Station index {station_idx} out of range (0-{dataset.dims['station']-1})")
        ...
        if boundary_idx >= self.grid.pylibs_hgrid.nob:
            raise ValueError(f"Boundary index {boundary_idx} out of range (0-{self.grid.pylibs_hgrid.nob-1})")
    ...
    if constituent is None:
        ...
        constituents = [c for c in major_constituents if c in dataset.constituent.values]
    ...
    for i, const in enumerate(constituents):
        ...
        try:
            if 'station' in dataset.dims:
                amp = dataset['amplitude'].isel(constituent=const_idx, station=station_idx).values
                phase = dataset['phase'].isel(constituent=const_idx, station=station_idx).values
            else:
            ...
            ax.plot([0, x], [0, y], color=color, linewidth=2, 
                   marker='o', markersize=8, label=const)
            
            # Add circle to represent amplitude
            circle = plt.Circle((0, 0), amp, fill=False, color=color, linestyle='--', alpha=0.4)
            ax.add_patch(circle)
...
def plot_tidal_dataset(self, figsize=(12, 8)):
    ...
    if 'station' in dataset.dims:
        ...
        ax1.text(0.5, 0.5, 'No station coordinates available', 
    ...
    if 'amplitude' in dataset and 'constituent' in dataset:
        ...
        ax2.text(0.5, 0.5, 'No amplitude or constituent data available', 
