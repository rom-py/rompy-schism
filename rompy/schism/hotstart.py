...
class SCHISMDataHotstart(DataGrid):
    ...
    time_var: str = Field(
        "time", description="Name of time variable in source dataset"
    )
    ...
    def get(self, 
            ...
            grid: SCHISMGrid,
            time: Optional[TimeRange] = None) -> str:
        """
        ...
        start_t = datenum(target_time.year, target_time.month, target_time.day,
        ...
        if self.time_var in ds.dims or self.time_var in ds.coords:
            ...
            if hasattr(ds[self.time_var].values[0], 'astype'):
                ...
                if hasattr(time_values[0], 'tolist'):
                    time_values = time_values.astype(float) / 24.0 + datenum(
                        self.time_base.year, self.time_base.month, self.time_base.day
                    ) + self.time_offset
            else:
                # Datetime values
            ...
            logger.info(f"Using time index {time_idx} (closest to requested start time)")
            
            # Select the data at this time
            if self.time_var in ds.dims:
                ds = ds.isel({self.time_var: time_idx})
        else:
            logger.warning(f"Time variable '{self.time_var}' not found in dataset. Using all data.")
        ...
        if hasattr(gd, 'ns'):
            ...
            ns = int(ne * 1.5)
            logger.warning(f"Could not determine number of sides, using estimated value: {ns}")
        ...
        for k in range(nvrt):
            ...
            for m, svar in enumerate(svars):
                ...
                if len(ds[svar].dims) == 4:  # time, depth, lat, lon
                    ...
                    raise ValueError(f"Unexpected dimensions for variable {svar}: {ds[svar].dims}")
                    ...
                    if np.any(fpn):
                        ...
                        if np.sum(~fpn) > 0:  # Only if we have valid points
                            v0[n, fpn] = sp.interpolate.griddata(
                                bxyz[~fpn, :], v0[n, ~fpn], bxyz[fpn, :], 'nearest', rescale=True
                            )
        ...
        # Define variables
        nd.vars = [
            'time', 'iths', 'ifile', 'idry_e', 'idry_s', 'idry', 'eta2', 'we', 'tr_el',
            'tr_nd', 'tr_nd0', 'su2', 'sv2', 'q2', 'xl', 'dfv', 'dfh', 'dfq1', 'dfq2',
            'nsteps_from_cold', 'cumsum_eta'
        ]
        
        # Initialize variables
        vi = zdata(); vi.dimname = ('one',); vi.val = np.array(0.0); nd.time = vi
        vi = zdata(); vi.dimname = ('one',); vi.val = np.array(0).astype('int'); nd.iths = vi
        vi = zdata(); vi.dimname = ('one',); vi.val = np.array(1).astype('int'); nd.ifile = vi
        vi = zdata(); vi.dimname = ('one',); vi.val = np.array(0).astype('int'); nd.nsteps_from_cold = vi
        
        vi = zdata(); vi.dimname = ('elem',); vi.val = np.zeros(ne).astype('int32'); nd.idry_e = vi
        vi = zdata(); vi.dimname = ('side',); vi.val = np.zeros(ns).astype('int32'); nd.idry_s = vi
        vi = zdata(); vi.dimname = ('node',); vi.val = np.zeros(np_grid).astype('int32'); nd.idry = vi
        vi = zdata(); vi.dimname = ('node',); vi.val = np.zeros(np_grid); nd.eta2 = vi
        vi = zdata(); vi.dimname = ('node',); vi.val = np.zeros(np_grid); nd.cumsum_eta = vi
        
        vi = zdata(); vi.dimname = ('elem', 'nVert'); vi.val = np.zeros([ne, nvrt]); nd.we = vi
        vi = zdata(); vi.dimname = ('side', 'nVert'); vi.val = np.zeros([ns, nvrt]); nd.su2 = vi
        vi = zdata(); vi.dimname = ('side', 'nVert'); vi.val = np.zeros([ns, nvrt]); nd.sv2 = vi
        vi = zdata(); vi.dimname = ('node', 'nVert'); vi.val = np.zeros([np_grid, nvrt]); nd.q2 = vi
        vi = zdata(); vi.dimname = ('node', 'nVert'); vi.val = np.zeros([np_grid, nvrt]); nd.xl = vi
        vi = zdata(); vi.dimname = ('node', 'nVert'); vi.val = np.zeros([np_grid, nvrt]); nd.dfv = vi
        vi = zdata(); vi.dimname = ('node', 'nVert'); vi.val = np.zeros([np_grid, nvrt]); nd.dfh = vi
        vi = zdata(); vi.dimname = ('node', 'nVert'); vi.val = np.zeros([np_grid, nvrt]); nd.dfq1 = vi
        vi = zdata(); vi.dimname = ('node', 'nVert'); vi.val = np.zeros([np_grid, nvrt]); nd.dfq2 = vi
        
        vi = zdata(); vi.dimname = ('elem', 'nVert', 'ntracers'); vi.val = tr_el; nd.tr_el = vi
        vi = zdata(); vi.dimname = ('node', 'nVert', 'ntracers'); vi.val = tr_nd; nd.tr_nd = vi
        vi = zdata(); vi.dimname = ('node', 'nVert', 'ntracers'); vi.val = tr_nd; nd.tr_nd0 = vi
