...
def analyze_grid_object(hgrid_obj):
    ...
    logger.info(f"Grid object attributes: {[attr for attr in dir(hgrid_obj) if not attr.startswith('__')]}")
    ...
    if hasattr(hgrid_obj, 'get_open_boundary_nodes'):
        try:
            ...
            logger.info(f"Method 1 SUCCESS: get_open_boundary_nodes() returned {len(boundaries)} boundaries")
    ...
    if hasattr(hgrid_obj, 'open_boundaries'):
        try:
            ...
            if boundaries:
                logger.info(f"Method 2 SUCCESS: open_boundaries attribute has {len(boundaries)} boundaries")
                for i, nodes in enumerate(boundaries):
                    logger.info(f"  Boundary {i} has {len(nodes)} nodes")
                return "Method 2: open_boundaries attribute"
            else:
                logger.info("Method 2 EMPTY: open_boundaries attribute exists but is empty")
        except Exception as e:
    ...
    if hasattr(hgrid_obj, 'boundaries'):
        ...
        logger.info(f"Boundaries attributes: {[attr for attr in dir(boundaries_obj) if not attr.startswith('__')]}")
        
        if hasattr(boundaries_obj, 'open_bnds'):
            try:
                boundaries = boundaries_obj.open_bnds
                logger.info(f"Method 3a SUCCESS: boundaries.open_bnds has {len(boundaries)} boundaries")
        ...
        if hasattr(boundaries_obj, 'get_boundary_nodes'):
            try:
                ...
                logger.info(f"Method 3b SUCCESS: boundaries.get_boundary_nodes() returned {len(boundaries)} boundaries")
                for i, nodes in enumerate(boundaries):
                    logger.info(f"  Boundary {i} has {len(nodes)} nodes")
                return "Method 3b: boundaries.get_boundary_nodes()"
            except Exception as e:
                logger.error(f"Method 3b FAILED: boundaries.get_boundary_nodes() - {str(e)}")
        else:
    ...
    if hasattr(hgrid_obj, 'mesh'):
        ...
        logger.info(f"Mesh attributes: {[attr for attr in dir(mesh) if not attr.startswith('__')]}")
        
        if hasattr(mesh, 'boundaries') and mesh.boundaries:
            try:
                boundaries = mesh.boundaries
                logger.info(f"Method 4 SUCCESS: mesh.boundaries has {len(boundaries)} boundaries")
                for i, boundary in enumerate(boundaries):
                    if hasattr(boundary, 'nodes'):
                        logger.info(f"  Boundary {i} has {len(boundary.nodes)} nodes")
                    else:
