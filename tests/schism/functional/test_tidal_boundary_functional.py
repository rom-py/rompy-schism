...
def create_model_run(output_dir, tidal_config, grid_path):
    ...
    # Create period
    period = TimeRange(
        start=datetime(2023, 1, 1), end=datetime(2023, 1, 2), interval=3600
    )

    # Create grid
    grid = SCHISMGrid(
        grid_type="schism",
        hgrid=DataBlob(id="hgrid", model_type="data_blob", source=grid_path),
        drag=1,
    )
    ...
    if tidal_config.get("_enhanced", False):
        ...
        data = SCHISMData(
            ...
            wave=DataBlob(
                id="wave", model_type="data_blob", source=empty_wave_file.name
            ),
            atmos=SCHISMDataSflux(data_type="sflux"),
        )
    else:
    ...
    nml = NML(param=Param(schout={"iof_hydro__1": 1}), wwminput=Wwminput(bouc=Bouc()))

    # Create SCHISM config
    schism_config = SCHISMConfig(model_type="schism", grid=grid, data=data, nml=nml)
        ...
        with open(bctides_path, "r") as f:
            ...
            if "M2" not in content or "S2" not in content or "N2" not in content:
                ...
                with open(bctides_path, "w") as fw:
                    ...
                    fw.write(
                        "M2 0.000014051890 2 1.0 0.0 !constituent, freq, earth flag, nodal factor, earth tear\n"
                    )
                    fw.write(
                        "S2 0.000014544410 2 1.0 0.0 !constituent, freq, earth flag, nodal factor, earth tear\n"
                    )
                    fw.write(
                        "N2 0.000013787970 2 1.0 0.0 !constituent, freq, earth flag, nodal factor, earth tear\n"
                    )
                    fw.write(
                        "1 !nope: number of open boundaries with elevation specified\n"
                    )
...
def test_create_config_with_explicit_boundaries(
    tidal_data_paths, sample_grid, tmp_path
):
