...
class TestBoundaryConditionsHotstartIntegration:
    ...
    def hycom_coords(self):
        ...
        return DatasetCoords(t="time", x="xlon", y="ylat", z="depth")
    ...
    def boundary_setup_with_sources(self, hycom_source, hycom_coords):
        ...
        salt_source = SCHISMDataBoundary(
            source=hycom_source, variables=["salinity"], coords=hycom_coords
        )
    ...
    def boundary_setup_no_sources(self):
        ...
        return BoundarySetupWithSource(
            elev_type=1, vel_type=1, temp_type=1, salt_type=1  # Tidal only
        )

    def test_boundary_conditions_without_hotstart(
        self, boundary_setup_with_sources, tidal_dataset
    ):
    ...
    def test_boundary_conditions_with_disabled_hotstart(
        self, boundary_setup_with_sources, tidal_dataset
    ):
    ...
    def test_boundary_conditions_with_enabled_hotstart(
        self, boundary_setup_with_sources, tidal_dataset
    ):
    ...
    def test_generate_hotstart_method(
        self, boundary_setup_with_sources, tidal_dataset, grid3d, time_range
    ):
    ...
    def test_generate_hotstart_no_temp_source(
        self, boundary_setup_no_sources, tidal_dataset, grid3d, time_range
    ):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            with pytest.raises(
                ValueError,
                match="Hotstart generation requires both temperature and salinity sources",
            ):
    ...
    def test_multiple_boundaries_hotstart_source_selection(
        ...
        boundary_0 = BoundarySetupWithSource(
            elev_type=1, vel_type=1, temp_type=1, salt_type=1
        )
    ...
    def test_hotstart_with_custom_variable_names(
        self, boundary_setup_with_sources, tidal_dataset, grid3d, time_range
    ):
    ...
    def test_hotstart_file_structure(
        self, boundary_setup_with_sources, tidal_dataset, grid3d, time_range
    ):
    ...
    @pytest.mark.slow
    def test_end_to_end_boundary_conditions_with_hotstart(
        self, boundary_setup_with_sources, tidal_dataset, grid3d, time_range
    ):
    ...
    def test_boundary_conditions_hotstart_disabled_no_generation(
        self, boundary_setup_with_sources, tidal_dataset, grid3d, time_range
    ):
