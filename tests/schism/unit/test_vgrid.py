...
class TestVgridGenerator:
    ...
    @pytest.mark.parametrize("ivcor,nvrt", [
        (2, 20),  # SZ vertical coordinate
        pytest.param(1, 30, marks=pytest.mark.skip(reason="ivcor=1 option not available yet")),   # LSC2 
    ])
    ...
    def test_vgrid_lsc2(self, hgrid_path, tmp_path):
        ...
        vgrid_generator = VgridGenerator(
            vgrid_type=VGRID_TYPE_LSC2,
            nvrt=15,
            hsm=20.0  # LSC2 specific
        )
